...
t_s = int(sys.argv[2]) # training set size in percentage
...
batch_size = int(float(t_s)*len(H)/100.0)
...
def train_network(num_epochs,state_size=N, verbose=True):
    with tf.Session() as sess:
        ...
        training_losses = []
        if os.path.isfile(folder+"model_3_f_"+str(t_s)+"_stacked"+str(N)+".ckpt"):
            saver.restore(sess, folder+"model_3_f_"+str(t_s)+"_stacked"+str(N)+".ckpt")
            ...
            saver.save(sess, folder+"model_3_f_"+str(t_s)+"_stacked"+str(N)+".ckpt")
...
def test_network():
    ...
    lamb_loss = t_get_integral(t_lamb_highest_states) - t_log_lambda(t_lambdas)
